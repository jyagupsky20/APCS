...
public class BST
{
  ...
  /**
    * void remove( int )
    * removes node containing val from BST
  */
  public void remove( int val ) {
    //First step: find the node
    TreeNode current = this._root; //Root we are currently at
    TreeNode before = null; //The previous value of current
    boolean relativePosition = false; //If it's false, current is to the left of before. If it's true, current is to the right.
    while(current.getValue() != val) {
      if (current == null) {
        return; // Our path brought us to a null node, which means the node isn't there.
      }
      before = current; //Update the before before we move the current
    ...
    //If we've made it this far, there's a node we can delete, which is stored at current, and its parent is stored at before
    //Now we will see which case we're in:
    if (current.getLeft() == null && current.getRight() == null) {
      //The node is a leaf, and can be deleted safely
      if (before == null) {
        this._root = null;
      }
      ...
      /*The node has two children. We'll set the head of the previous node to the root of the right subtree, and set
      the head of the left subtree as the left child of the leftmost element of the right subtree.
      */
      TreeNode rightRoot = current.getRight();
      TreeNode leftRoot = current.getLeft();
      current = current.getRight(); //Start traversing the right subtree
      while (current.getLeft() != null) {
        current = current.getLeft(); //Get the leftmost element
      }
      current.setLeft(leftRoot); //Set current's left to the root of the left subtree.
      if (before == null) {
        this._root = rightRoot;
      }
      else if (relativePosition) {
        before.setRight(rightRoot);
      }
      else {
        before.setLeft(rightRoot);
      }
    }
    else {
